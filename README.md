...
A rede é representada por um grafo dinâmico:
$$
\mathcal{G}(t) = (\mathcal{V}, \mathcal{E}(t))
$$

onde:
- $\mathcal{V} = \{v_1, v_2, \dots, v_N\}$ é o conjunto fixo de nós da rede;
- $\mathcal{E}(t) \subseteq \mathcal{V} \times \mathcal{V}$ é o conjunto de enlaces ativos no ciclo discreto $t \in \mathbb{N}$.

### Vizinhanças Ativa e Passiva

Cada nó $v_i \in \mathcal{V}$ mantém dois conjuntos de vizinhança disjuntos:

**Active View**
$$
\mathcal{A}_i(t) \subset \mathcal{V} \setminus \{v_i\}, \qquad |\mathcal{A}_i(t)| \leq A_{\max}
$$

**Passive View**
$$
\mathcal{P}_i(t) \subset \mathcal{V} \setminus \bigl(\{v_i\} \cup \mathcal{A}_i(t)\bigr), \qquad |\mathcal{P}_i(t)| \leq P_{\max}
$$

Assume-se que:
$$
\mathcal{A}_i(t) \cap \mathcal{P}_i(t) = \varnothing, \qquad A_{\max} \ll P_{\max}
$$

### Diversidade Histórica

Cada nó mantém um conjunto acumulativo que registra todos os vizinhos distintos já observados na Passive View:
$$
\mathcal{H}_i(t) = \bigcup_{\tau = 1}^{T_{\max}} \mathcal{P}_i(\tau)
$$

### Dinâmica por Ciclo

A operação da camada NetMaintenance ocorre em ciclos discretos. Em cada ciclo $t$ para todo nó $v_i$, executam-se as seguintes etapas:

**1. Envio de Pings e Modelo de Falha**

Para cada vizinho $v_j \in \mathcal{A}_i(t)$, define-se uma variável aleatória:
$$
X_{ij}(t) \sim \text{Bernoulli}(1 - p_j),
$$
onde $X_{ij}(t) = 1$ indica resposta ao ping e $X_{ij}(t) = 0$ indica falha.

A probabilidade de falha do nó $v_j$ é modelada como:
$$
p_j \sim \mathcal{N}(\mu_f, \sigma_f^2), \quad p_j \in [0,1]
$$

**2. Remoção e Substituição de Vizinhos Falhos**

Define-se o conjunto de falhas:
$$
\mathcal{F}_i(t) = \{v_j \in \mathcal{A}_i(t) \mid X_{ij}(t) = 0\}
$$

Os nós falhos são removidos:
$$
\mathcal{A}_i'(t) = \mathcal{A}_i(t) \setminus \mathcal{F}_i(t)
$$

Para cada vaga, seleciona-se um candidato $v_k \in \mathcal{P}_i(t)$, testado segundo:
$$
X_{ik}(t) \sim \text{Bernoulli}(1 - p_k)
$$

Se o teste for bem-sucedido:
$$
\mathcal{A}_i(t^+) = \mathcal{A}_i'(t) \cup \{v_k\}, \quad \mathcal{P}_i(t^+) = \mathcal{P}_i(t) \setminus \{v_k\}
$$

O processo continua até que $|\mathcal{A}_i(t^+)| = A_{\max}$ ou não existam candidatos disponíveis.

**3. Troca Parcial da Passive View (Shuffle)**

Cada nó $v_i$ seleciona aleatoriamente um parceiro $v_j \in \mathcal{A}_i(t^+)$ e define um subconjunto:
$$
\mathcal{S}_i(t) \subseteq \mathcal{P}_i(t^+),
\qquad |\mathcal{S}_i(t)| = \lfloor \alpha |\mathcal{P}_i(t^+)| \rfloor,
$$
onde $\alpha \in (0,1)$ representa a fração de troca (neste caso $\alpha = 0.3$).

Os subconjuntos $\mathcal{S}_i(t)$ e $\mathcal{S}_j(t)$ são trocados, respeitando os limites de tamanho $P_{\max}$.

**4. Atualização da Diversidade Histórica**

Após o shuffle, atualiza-se:
$$
\mathcal{H}_i(t+1) = \mathcal{H}_i(t) \cup \mathcal{P}_i(t+1)
$$

A métrica de diversidade histórica do nó $v_i$ no ciclo $t$ é definida como:
$$
D_i(t) = |\mathcal{H}_i(t)|
$$
